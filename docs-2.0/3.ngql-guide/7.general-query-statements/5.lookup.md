...
- 如果用`LOOKUP`语句基于指定属性查询时该属性没有索引，系统会在可用的索引中随机选择一个。
  ...
  例如，Tag `player` 有属性 `name` 和 `age`，Tag `player` 本身和属性 `age` 有索引，而属性 `name` 没有索引。当运行 `LOOKUP ON player WHERE player.age == 36 YIELD player.name;` 时，系统会在 Tag `player` 和属性 `age` 的索引中随机使用一个。
