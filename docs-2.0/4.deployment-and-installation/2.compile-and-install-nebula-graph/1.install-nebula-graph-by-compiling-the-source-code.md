...
1. 克隆Nebula Graph v{{ nebula.release }}版本的源代码到主机。
...
4. 运行如下命令，使用CMake生成makefile文件。
  ...
  为了让相关库的版本保持一致，需使用选项`-DNEBULA_COMMON_REPO_TAG`和`-DNEBULA_STORAGE_REPO_TAG`指定[nebula-common](https://github.com/vesoft-inc/nebula-common)和[nebula-storage](https://github.com/vesoft-inc/nebula-storage)库的分支。
